...
def test_composition(offset=0, f=85, aperture=75, edge_thickness=3, biconvex=False, draw_spot_diagram=True, save=False, add_telescope=False, add_text="", add_propagation=0, prop_to_second_pump=False):
  ...
  if prop_to_second_pump:
    final_spot_size = pump_spot_size
    Lens2 = Thicklens(f=f, n=1.515, aperture=aperture, biconvex=biconvex)
    Comp.propagate(17) # distance between the two laser media image planes
    Comp.propagate(image_distance+Lens2.h1)
    Comp.add_on_axis(Lens2)
    Comp.propagate(object_distance+Lens2.h2)
  ...
  if draw_spot_diagram and not freecad_da: 
    point_x, point_y = IP2.spot_diagram(Comp._beams[-1], save=save, filename=file_name, title=title, draw_rectangle=True, rectangle_size=(final_spot_size, final_spot_size), default_diagram_size=final_spot_size)
